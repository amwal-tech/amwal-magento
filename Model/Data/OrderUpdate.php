...
class OrderUpdate
{
    ...
    private function dataValidation(Order $order, DataObject $amwalOrderData)
    {
        ...
        if ((float)$order->getTotalDue() != (float)$amwalOrderData->getTotalAmount()) {
            $this->sendAdminEmail($order, __('Order (%1) needs Attention', $order->getIncrementId()), $this->dataValidationMessage($order->getIncrementId(), 'total_due', 'total_amount', $order->getTotalDue(), $amwalOrderData->getTotalAmount()));
