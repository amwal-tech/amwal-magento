...
class OrderUpdate
{
    ...
    private function dataValidation(Order $order, DataObject $amwalOrderData)
    {
        ...
        if (floatval($order->getGrandTotal()) != floatval($amwalOrderData->getTotalAmount())) {
            $this->sendAdminEmail($order, 'Order (%s) needs Attention', $this->dataValidationMessage($order->getIncrementId(), 'grand_total', 'total_amount', $order->getGrandTotal(), $amwalOrderData->getTotalAmount()));
